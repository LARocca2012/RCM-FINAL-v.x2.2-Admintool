...
$db = new PDO('sqlite:'.$cpath . 'ReCodMod/databases/db1.sqlite');
  ...
  if(($adm_ip == $i_ipo) && (($a_grp == 0) || ($a_grp == 111)))
   {
...
AddToLog("[".$datetime."] BANNED IP KICK: (" . $i_ip . ") (" . $i_name . ")");		
           ...
           ++$x_stop_lp;			
		++$x_number;
		echo ' bans   '.(microtime(true) - $start); exit;
		}	
	}	
...
++$x_stop_lp;		  //echo $namez[0];       
	//echo ' proxod3   '.(microtime(true) - $start);	
		}
      else
        {
       // echo 'No match found';
	   //echo ' vooooooords   '.(microtime(true) - $start);
        }
    }	
...
++$x_stop_lp;		echo ' x-rangeee   '.(microtime(true) - $start);	
	}
