...
$now_is=date('YmdHis');
$fhrf=file($log_cash."/temp-d2.txt");
foreach ($fhrf as $flvr) {
$flmm=explode("%",$flvr);
$res3=$now_is-$flmm[1];
}
$fhrz=file($log_cash."/temp-c2.txt");
foreach ($fhrz as $flix) {
$flmz=explode("%",$flix);
$res2=$now_is-$flmz[1];
}
$fhr=file($log_cash."/temp-b2.txt");
foreach ($fhr as $flinx) {
$flmc=explode("%",$flinx);
$res1=$now_is-$flmc[1];
}
$fh=file($log_cash."/temp-a2.txt");
foreach ($fh as $fline) {
$flm=explode("%",$fline);
$res=$now_is-$flm[1];
  ...
  || $flm[0]==$x_n3 && $flm[0]==$x_n4 && $flmz[2]==$msgrn and $res2<=$flood_time_cmd2) && ($x_number != 1))
//  if (($flm[0]==$x_n3 && $flm[0]==$x_n4 and $res<=$flood_time 
//  || $flm[0]==$x_n3 && $flm[0]==$x_n4 && substr($flmz[2], 0, 8)==substr($msgrn, 0, 8) and $res2<=$flood_time_cmd) && ($x_number != 1))
{ echo " FLUD "; 
usleep($sleep_rcon);
...
$x_n4 = trim(clearnamex($nivv)); 	
$today=date('YmdHis');
$fh=fopen($log_cash."/temp-b2.txt" ,"w+");
fwrite($fh, $x_n4.'%'.$today);
fclose($fh);

$fh=fopen($log_cash."/temp-d2.txt" ,"w+");
fwrite($fh, $x_n4.'%'.$today.'%'.$msgrn);
fclose($fh);
 

  if (($flm[0]==$x_n3 && $flm[0]==$x_n4 && $flmc[0]==$x_n3 and $res<=$flood_time
  || $flm[0]==$x_n3 && $flm[0]==$x_n4 && $flmm[2]==$msgrn and $res2<=$flood_time_cmd2) && ($x_number != 1))  
// if (($flm[0]==$x_n3 && $flm[0]==$x_n4 && $flmc[0]==$x_n3 and $res<=$flood_time
//  || $flm[0]==$x_n3 && $flm[0]==$x_n4 && substr($flmm[2], 0, 8)==substr($msgrn, 0, 8) and $res2<=$flood_time_cmd) && ($x_number != 1))  
{ echo " FLUD "; 
...
sqlite_query($db, "UPDATE x_db_players SET x_db_warn=x_db_warn +1 WHERE x_db_ip='{$i_ip}'"); 
usleep($sleep_rcon);
rcon('clientkick '. $i_id, '');}
else { usleep($sleep_rcon); rcon('akick '. $i_id.' " ^6[^7BANNED - CHAT FLOOD!^6]"', ''); }
$i_namex = aaxa($i_name);
